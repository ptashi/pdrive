...
export default function DocumentsPage() {
  ...
  const documents = files.filter((f) => f.type === "pdf");
  const totalBytes = documents.reduce((sum, f) => sum + f.sizeBytes, 0);

  return (
    <DashboardShell active="Documents" onUploadComplete={loadFiles}>
      <h1 className="text-2xl font-bold text-brand-dark mb-1">Documents</h1>
      <p className="text-accent text-sm mb-6">Total: {formatSize(totalBytes)}</p>

      {loading ? (
        <p className="text-brand-dark">Loading...</p>
      ) : documents.length === 0 ? (
        <p className="text-accent">No documents uploaded yet.</p>
      ) : (
