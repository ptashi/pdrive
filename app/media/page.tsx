...
export default function MediaPage() {
  ...
  const media = files.filter((f) => f.type === "video" || f.type === "audio");
  const totalBytes = media.reduce((sum, f) => sum + f.sizeBytes, 0);

  return (
    <DashboardShell active="Media" onUploadComplete={loadFiles}>
      <h1 className="text-2xl font-bold text-brand-dark mb-1">Media</h1>
      <p className="text-accent text-sm mb-6">Total: {formatSize(totalBytes)}</p>

      {loading ? (
        <p className="text-brand-dark">Loading...</p>
      ) : media.length === 0 ? (
        <p className="text-accent">No media uploaded yet.</p>
      ) : (
